...
/// These following implementations are based on reflections. If you want to avoid reflections, you have to provide
/// your own implementations for:
///
...
///
///
/// In addition to manually configuring this builder, you can also provide implementations of [Extension] trough
/// Javas [`service
...
///
/// These implementations of [Extension] can be additionally configured by adding the according implementation of
///  [Extension.Data]
...
public class JDACBuilder {
    ...
    static {
        Proteus.global()
                .register(Type.of(Class.class), Type.of(String.class),
                          Mapper.uni((klass, _) -> MappingResult.lossless(klass.getName())));
    }
    ...
    JDACBuilder(JDAContext jdaContext) {
        ...
        addFallback(LOCALIZATION_FUNCTION, ctx -> new JDACLocalizationFunction(ctx.get(BUNDLE_FINDER),
                                                                               ctx.get(DEFINITIONS),
        ...
        addFallback(STRING_RESOLVER, ctx -> List.of(ctx.get(I18N), ctx.get(EMOJI_RESOLVER),
                                                    ctx.get(PLACEHOLDER_RESOLVER)));
        ...
        addFallback(
                MERGED_CLASS_FINDER, ctx -> annotationClass -> ctx.get(CLASS_FINDER)
                        .stream()
    ...
    private <T> JDACBuilder addUserProperty(Property<T> type, Function<PropertyProvider.Context, T> supplier) {
        ScopedValue.where(Properties.INSIDE_FRAMEWORK, true)
                .run(() -> properties.add(PropertyProvider.create(type, Properties.USER_PRIORITY, supplier)));
        return this;
    }
    ...
    /// If you want to add own [ClassFinder]s while keeping the default reflective implementation, you have to add it
    ///  explicitly via
    ...
    /// @apiNote This method overrides the default/fallback value but adds to the loaded ones.
    /// If you want to add own [EmojiSource]s while keeping the default reflective implementation, you have to add it
    ///  explicitly via
    ...
    public JDACBuilder extensionData(Extension.Data... data) {
        return addUserProperty(
                EXTENSION_DATA, _ -> Arrays.stream(data)
    ...
    /// Specifies a way to filter found implementations of [Extension] if you have clashing or cycling dependencies
    /// for example.
    ///
    /// @param strategy the filtering strategy to be used either [ExtensionFilter.FilterStrategy#INCLUDE] or
    ///  [ExtensionFilter.FilterStrategy#EXCLUDE]
    /// @param classes  the classes to be filtered
    /// @apiNote This method compares the [`fully classified class name`][Class#getName()] of all [Extension]
    ///  implementations by using [String#startsWith(String)],
    /// so it's possible to include/exclude a bunch of extensions sharing the same base package by just providing the
    ///  package name.
    /// @see ExtensionFilter
    ...
    public JDACommands start() {
        ...
        IntrospectionImpl introspection = new IntrospectionImpl(new Lifecycle(), properties.createResolver(),
                                                                Stage.CONFIGURATION);
        ...
        return ScopedValue.where(IntrospectionImpl.INTROSPECTION, introspection).call(() -> {
            try {
                ...
                Middlewares middlewares = new Middlewares(introspection.get(MIDDLEWARE),
                                                          introspection.get(ERROR_MESSAGE_FACTORY),
                                                          introspection.get(PERMISSION_PROVIDER));
                TypeAdapters typeAdapters = new TypeAdapters(introspection.get(TYPE_ADAPTER),
                                                             introspection.get(MESSAGE_RESOLVER));

                IntrospectionImpl initIntrospection = Properties.Builder.newRestricted()
                        .addFallback(
                                INTERACTION_REGISTRY, ctx -> new InteractionRegistry(
