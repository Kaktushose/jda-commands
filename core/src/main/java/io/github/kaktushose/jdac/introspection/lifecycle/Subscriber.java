...
/// A [Subscriber] holds logic that will be executed when a certain [Event] is published by the framework.
///
/// @param <T> the subscribed to [Event]
///
...
public interface Subscriber<T extends Event> extends BiConsumer<T, Introspection> {
    ...
    /// Executed when the subscribed to [Event] ([T]) is published by the framework.
    ///
    ...
    /// Thus, the [stage][Introspection#currentStage()] of the [Introspection] is depending on the used [Event].
    /// You can take a look at the specific [IntrospectionAccess] annotation of an [Event] class to know which stage is available.
    ///
    /// This method will be executed in different threads, thus the logic inside needs to be threadsafe!
    ///
    /// @param t the published [Event]
