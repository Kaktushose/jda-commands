...
/// An [InteractionControllerInstantiator] is used get instances of classes annotated with [Interaction], if needed
/// creating those.
///
/// Please also note that per [`Runtime`]({@docRoot}/index.html#runtime-concept-heading) there can be multiple
/// classes annotated with [Interaction] but there can be only one instance per class of those per
